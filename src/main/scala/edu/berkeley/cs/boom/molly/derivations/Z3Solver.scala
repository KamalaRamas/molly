...
object Z3Solver extends Solver {
  ...
  protected def solve(failureSpec: FailureSpec, goal: GoalNode, firstMessageSendTimes: Map[String, Int], seed: Set[SolverVariable])(implicit metrics: MetricBuilder): Traversable[Set[SolverVariable]] = {
    ...
    val importantNodes: Set[String] = goal.importantClocks.filter(_._3 < failureSpec.eot).map(_._1).toSet ++ seed.collect {
      case cf: CrashFailure => cf.node
    }
    ...
    lazy val goalToZ3: GoalNode => Z3AST = Memo.mutableHashMapMemo(goalToZ3Internal(_))
    ...
    assertAtLeastK(solver, failureSpec.nodes.map(NeverCrashed).map(varToZ3), failureSpec.nodes.size - failureSpec.maxCrashes)
