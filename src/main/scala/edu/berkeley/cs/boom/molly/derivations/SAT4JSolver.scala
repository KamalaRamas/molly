...
object SAT4JSolver extends Solver {
  ...
  protected def solve(failureSpec: FailureSpec, goal: GoalNode, firstMessageSendTimes: Map[String, Int], seed: Set[SolverVariable])(implicit metrics: MetricBuilder): Traversable[Set[SolverVariable]] = {
    ...
    implicit def satVarsToVecInt(clause: Iterable[SolverVariable]): IVecInt = new VecInt(clause.map(satVarToInt).toArray)
    ...
    val importantNodes: Set[String] = formula.root.vars.filter(_._3 < failureSpec.eot).map(_._1).toSet ++ seed.collect {
